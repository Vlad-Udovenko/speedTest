...
function liteChart(id, settings) {
	...
	let avar = document.getElementById("avarage")

	if(undefined != element) {
		element.remove();
	}
	if(avar){
		avar.parentNode.removeChild(avar);
	};
	...
	let drawLegendTable = function(chart) {
		...
		chart.legends.list.forEach(function(legend) {
			...
			if("vertical" == tableOptions.direction) {
				...
				legendMark = line( "-mark-" + legendIndex, x, linePosition + 6, x + 12, linePosition + 6, legend.fill, legend.stroke, 4);
				g.appendChild(legendMark);
				objectArray.push(legendMark);

				x += labelOptions.fontSize + margin;

				legendText = createText("-text-" + legendIndex
					, x
					, linePosition
					, labelOptions.fontSize
					, "start"
